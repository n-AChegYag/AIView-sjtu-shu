...
def main():
    ...
    model = torch.load('/workspace/ct_model.pth')['model']
    model = model.cuda()
...
def pred(model, args):
    ...
    from_transforms = torch.nn.Upsample((512, 512, 512), mode="trilinear", align_corners=False)
    ...
    for f_name in os.listdir(args.input_dir):
        ...
        if args.mode == 'sample':
            ...
            sample_score = (sample_score - 0.0158) / 0.001
            ...
            pixel_scores = inv_transform(pixel_scores, aff_transform, sitk.sitkNearestNeighbor, 'abdom', mov_info)
